...
from tdi.pipeline import TechnologyDriversIdentificationPipeline  # noqa: E402
...
async def main() -> int:
    ...
    out_path = Path(args.output) if args.output else (
        ROOT / "output" / f"{slugify(args.query)}_{args.year}.json"
    )
    out_path.parent.mkdir(parents=True, exist_ok=True)

    print("Running Technology Drivers Identification…")
